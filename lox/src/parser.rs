...
impl<'a> Parser<'a> {
	...
	fn consume_var_declaration(&mut self) -> StmtBox<'a> {
		if self.try_consume(TokenType::IDENTIFIER).is_none() {
			return Err(ParseError{token: &self.tokens[self.index], message: "Expected variable name."});
		}
		...
		if self.try_consume(TokenType::SEMICOLON).is_none() {
			return Err(ParseError{token: &self.tokens[self.index], message: "Expected ';' after variable declaration."});
		}
		Ok(Box::new(Stmt::Var{name: &self.tokens[self.index], initializer}))
	...
	fn consume_print_stmt(&mut self) -> StmtBox<'a> {
		...
		if self.try_consume(TokenType::SEMICOLON).is_none() {
			return Err(ParseError{token: &self.tokens[self.index], message: "Expected ';' after value."});
		}
	...
	fn consume_expression_stmt(&mut self) -> StmtBox<'a> {
		let e = self.consume_expression()?;
		if self.try_consume(TokenType::SEMICOLON).is_none() {
			return Err(ParseError{token: &self.tokens[self.index], message: "Expected ';' after expression."});
		}
	...
	fn consume_equality(&mut self) -> ExprBox<'a> {
		self.consume_left_associative_binary_operators(Self::consume_comparison, &[TokenType::EQUAL_EQUAL, TokenType::BANG_EQUAL])
	}
	fn consume_comparison(&mut self) -> ExprBox<'a> {
		self.consume_left_associative_binary_operators(Self::consume_term, &[TokenType::LESS, TokenType::LESS_EQUAL, TokenType::GREATER, TokenType::GREATER_EQUAL])
	}
	fn consume_term(&mut self) -> ExprBox<'a> {
		self.consume_left_associative_binary_operators(Self::consume_factor, &[TokenType::MINUS, TokenType::PLUS])
	}
	fn consume_factor(&mut self) -> ExprBox<'a> {
		self.consume_left_associative_binary_operators(Self::consume_unary, &[TokenType::STAR, TokenType::SLASH])
	}
	...
	fn consume_primary(&mut self) -> ExprBox<'a> {
		...
		match tok.token_type {
			TokenType::FALSE | TokenType::TRUE | TokenType::NUMBER | TokenType::STRING | TokenType::NIL =>
				Ok(Box::new(Expr::Literal{value: tok.literal.as_ref().unwrap()})),
			...
			_ => {
				Err(ParseError{token: &tok, message: "Expected expression."})
			}
		}
	}
	fn consume_left_associative_binary_operators(&mut self, consumer: fn(&mut Parser<'a>) -> ExprBox<'a>, operators: &[TokenType]) -> ExprBox<'a> {
		...
		loop {
			...
			ret = Box::new(Expr::Binary{left: ret, operator: &operator, right});
		}
	}
	fn synchronize(&mut self) {
		while self.tokens[self.index].token_type != TokenType::EOF {
			...
			self.index += 1;
			if matches!(self.tokens[self.index].token_type, TokenType::CLASS | TokenType::FUN | TokenType::VAR
					| TokenType::FOR | TokenType::IF | TokenType::WHILE | TokenType::PRINT | TokenType::RETURN) {
				return;
			}
		}
	}

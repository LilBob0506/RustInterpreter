...
use crate::entities::{LiteralValue, Token, TokenType};
...
impl Scanner<'_> {
	pub fn new(source: &str) -> Scanner {
		assert!(source.is_ascii(), "Source code contains non ASCII characters. Aborting.");
		let ret = Scanner {
	...
	pub fn scan_tokens(&mut self) -> &Vec<Token> {
		...
		self.tokens.push(Token{token_type: TokenType::EOF, lexeme: "", line: self.line, literal: None});
		&self.tokens
	}
		...
		match c {
			...
			c => {
				if c.is_ascii_digit() {
					...
					if self.current + 1 < self.source.len() && self.source[self.current] == b'.' && self.source[self.current + 1].is_ascii_digit() {
						self.advance();
						self.consume_while(&u8::is_ascii_digit);
					};
					...
					let keyword = self.keywords.get(&std::str::from_utf8(&self.source[self.start..self.current]).unwrap());
	...
	fn add_token(&mut self, token_type: TokenType) {
		let lexeme;
		unsafe {
			lexeme = std::str::from_utf8_unchecked(&self.source[self.start..self.current]); // already asserted to be valid ascii
		}
